...
Todo {
  ...
  archivedAt?: string // ISO8601, 存在する場合はゴミ箱
  dueAt?: string      // ISO8601, 任意
...
* `archivedAt` が存在する Todo は **ゴミ箱（Trash）** に属する
* `priority` 未設定は、表示・ソート上 **`none`（low より低い）** として扱う
* 並び順は **manual order（orderKey）** により管理される

...
- **型の不一致が発生しやすい**: 今回の「完了状態」のように、ドメインモデルにない概念をUI側で表現する際にハック（COMPLETED_TAG_ID = -1）が必要になった。これは制約の副作用
...
- **完了状態の扱いに無理が見える**: 仕様にない「completedタグ」をTag型で表現するためにCOMPLETED_TAG_ID = -1というハックが必要だった。これはドメインモデルと実際のUIニーズの乖離を示している
